...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**ZE"):
            buttons = [
                (
                    Button.inline("Stats", data="stats"),
                    Button.url("DEV", "https://t.me/UI_XB"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./ZE", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./ZE", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="ZE Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب زد إي **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/UI_XB"),
                Button.url(
                    "المطور",
                    "t.me/UP_UO",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل زد إي من تنصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[𝗭𝞝 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 🧸♥](https://t.me/UI_XB)",
...
@ZE.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@ZE.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@ZE.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@ZE.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
