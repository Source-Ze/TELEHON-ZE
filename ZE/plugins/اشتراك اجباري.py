...
async def participant_check(channel, user_id):
    try:
        await ZE(GetParticipantRequest(channel, int(user_id)))
        return True
...
async def fsub(event):
    ...
    if not str(channel).startswith("@"):
        ...
        if not await participant_check(channel, ZE.uid):
            return await event.reply(
...
@ZE.ar_cmd(pattern="تعطيل الاجباري")
