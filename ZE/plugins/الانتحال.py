...
@ZE.ar_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
    mid = await ZE.get_me()
    ...
    if replied_user.id == 5650717789:
        return await edit_delete(event, "**انت عبيط ي صاحبيده مطور السورس 😂!**")
...
#Reda
ZE = ["UP_UO", "UI_XB"]
@ZE.ar_cmd(pattern="انتحال_الدردشه")
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        chat_id = msg
        try:
            result = await ZE(GetFullChannelRequest(
                chat_id
            ))
        except ValueError:
        ...
        mych = await ZE(GetFullChannelRequest(
                event.chat_id
            ))
        if msg in ZE:
        ...
        try:
            await ZE(functions.channels.EditTitleRequest(
                channel=await ZE.get_entity(event.chat_id),
        ...
        try:
            await ZE(functions.messages.EditChatAboutRequest(
            peer=event.chat_id,
            about=result.full_chat.about
        ))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        profile_pic = await ZE.download_profile_photo(chat_id, Config.TEMP_DIR)
        pfile = await ZE.upload_file(profile_pic)
        try:
            await ZE(functions.channels.EditPhotoRequest(event.chat_id, pfile))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
...
#Reda
@ZE.ar_cmd(pattern="اعادة_الدردشه")
async def reda_back(event):
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            ...
            async for photo in ZE.iter_profile_photos(event.chat_id, limit=1) :
                    await ZE(
