...
@ZE.ar_cmd(pattern="بحث(320)?(?:\s|$)([\s\S]*)")
...
@ZE.ar_cmd(pattern="(ا(ل)?ا(س)?م)(?:\s|$)([\s\S]*)")
    ...
    if flag == "s":
        ...
        async with event.client.conversation(chat) as conv:
            try:
                ...
                await ZE(unblock("DeezerMusicBot"))
                purgeflag = await conv.send_message("/start")
