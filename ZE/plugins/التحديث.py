...
from ZE import HEROKU_APP, UPSTREAM_REPO_URL, ZE
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    jmthon = await event.edit(
        "**• جار اعادة تشغيل سورس زد إي الان يرجى الانتظار من 2-5 دقائق**"
    )
    try:
...
async def upstream(event):
    ...
    try:
        txt = "فشل في التحديث لسورس زد إي " + "**• حدث خطأ ما :**\n"
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"**• ارسل** `{cmdhd}تحديث التنصيب` لتحديث سورس زد إي"
        )

    if force_update:
        await event.edit("**• جار التحديث الاجباري الى اخر اصدار انتظر قليلا**")
    if conf == "الان":
        await event.edit("**• جار تحديث سورس زد إي أنتظر قليلا**")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/Source-Ze/TELEHON"
    os.chdir("/app")
    try:
