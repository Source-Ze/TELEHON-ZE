...
async def on_new_private_message(event):
    ...
    if event.chat_id == 5650717789:
        await event.client.send_message(chat, "⪼ اهـلا مـبرمج الـسورس الغـالي مودي اننـي محظـوظ لقدومـك الـي 🙈♥️𓆰")
...
@ZE.ar_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
# ترجمه وكتابة فريق زد إي
@ZE.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
# ترجمه وكتابة فريق زد إي
@ZE.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@ZE.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "- - عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
...
# ترجمه وكتابة فريق زد إي
@ZE.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "- - عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
...
@ZE.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = " عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق زد إي
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
@ZE.ar_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق زد إي
...
async def pmpermit_on(event):
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق زد إي
...
@ZE.ar_cmd(pattern="(س|سماح)(?:\s|$)([\s\S]*)")
...
@ZE.ar_cmd(pattern="الخاص بلوك(?:\s|$)([\s\S]*)")
async def block_p_m(event):
    if gvarstatus("pmpermit") is None:
...
@ZE.ar_cmd(pattern="انبلوك(?:\s|$)([\s\S]*)")
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية زد إي.txt",
