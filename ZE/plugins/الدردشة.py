...
async def ban_user(chat_id, i, rights):
    try:
        await ZE(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@ZE.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@ZE.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
@ZE.on(events.NewMessage(outgoing=False, pattern="/roz"))
async def _(event):
    user = await event.get_sender()
    if user.id == 5650717789:
        await event.reply("اهلا بك مودي مطوري\nقناة السورس : @UI_OS")
...
@ZE.ar_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    ...
    try:
        await ZE(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in ZE.iter_participants(chat_id):
        ...
        await ZE.send_message(chat_id, usrtxt)
...
@ZE.ar_cmd(pattern="الغاء التفليش", groups_only=True)
