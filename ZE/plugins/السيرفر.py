...
from ..helpers.utils import ZEutils
...
async def switch_branch():
    with open(config, "r") as f:
        configs = f.read()
    BRANCH = "master"
    REPO = "https://github.com/Source-Ze/UI_XB50"
    EXTERNAL = False
...
@ZE.ar_cmd(pattern="(وضع|جلب|حذف) فار ([\s\S]*)")
        ...
        if variable not in var_checker:
            if variable == "EXTERNAL_REPO":
                if bool(value and (value.lower() != "false")) and not url(value):
                    value = "https://github.com/SOURCE-SPIDER/Plugins"
                else:
                    return await edit_or_reply(
                        jmthon,
