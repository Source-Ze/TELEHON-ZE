...
from . import edit_delete, ZE, logging
...
autopic_path = os.path.join(os.getcwd(), "ZE", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "ZE", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "ZE", "photo_pfp.png")
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await ZE.upload_file(autophoto_path)
        try:
            if i > 0:
                await ZE(
                    functions.photos.DeletePhotosRequest(
                        await ZE.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await ZE(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await ZE(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    while AUTOBIOSTART := gvarstatus("autobio") == "true":
        ...
        LOGS.info(bio)
        try:
            await ZE(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
@ZE.ar_cmd(pattern="الصورة الوقتية$")
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
...
ZE.loop.create_task(digitalpicloop())
ZE.loop.create_task(autoname_loop())
ZE.loop.create_task(autobio_loop())
